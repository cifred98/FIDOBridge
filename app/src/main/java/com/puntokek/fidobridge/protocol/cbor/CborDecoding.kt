...
private fun fromCbor(iter: Iterator<Byte>): CborValue? {
    with(iter) {
        ...
        val value: ULong = when (additionalInfo) {
            ...
            24.toByte() -> nextInteger(UByte.SIZE_BYTES)?.takeUnless { it <= 23.toULong() }
            25.toByte() -> nextInteger(UShort.SIZE_BYTES)?.takeUnless { it <= UByte.MAX_VALUE }
            26.toByte() -> nextInteger(UInt.SIZE_BYTES)?.takeUnless { it <= UShort.MAX_VALUE }
            27.toByte() -> nextInteger(ULong.SIZE_BYTES)?.takeUnless { it <= UInt.MAX_VALUE }
            else -> null
        } ?: return null

        return when (val majorType = (initialByte.toUByte().toUInt() shr 5).toUByte()) {
            MajorType.UNSIGNED_INTEGER.value -> {
                if (value <= Long.MAX_VALUE.toULong()) CborLong(value.toLong())
                else CborUnsignedInteger(value)
            }
            MajorType.NEGATIVE_INTEGER.value -> {
                if (value <= Long.MAX_VALUE.toULong()) CborLong(-1 - value.toLong())
            ...
            MajorType.BYTE_STRING.value, MajorType.TEXT_STRING.value -> {
                ...
                if (majorType == MajorType.BYTE_STRING.value) CborByteString(array)
                else CborTextString(array.decodeToString())
            }
            MajorType.ARRAY.value -> {
                if (value > Int.MAX_VALUE.toULong()) return null
                val array = Array(value.toInt()) { fromCbor(iter) ?: return null }
                CborArray(array)
            }
            MajorType.MAP.value -> {
                if (value > Int.MAX_VALUE.toULong()) return null
            ...
            MajorType.SIMPLE.value -> {
                when (additionalInfo) {
                    in 0..19 -> CborSimpleValue(additionalInfo.toUByte())
                    20.toByte() -> CborBoolean(false)
                    21.toByte() -> CborBoolean(true)
                    22.toByte() -> CborNull
                    23.toByte() -> CborUndefined
                    24.toByte() -> CborSimpleValue(value.toUByte()).takeUnless { value < 32.toULong() }
                    25.toByte() -> CborUndefined // TODO: half-precision float
                    26.toByte() -> CborFloatingPointNumber(Float.fromBits(value.toInt()).toDouble())
                    27.toByte() -> CborFloatingPointNumber(Double.fromBits(value.toLong()))
                    else -> null
                }
            }
            else -> null
