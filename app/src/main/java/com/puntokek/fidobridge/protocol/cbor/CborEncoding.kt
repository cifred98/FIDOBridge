...
@JvmInline
value class CborUnsignedInteger(override val value: ULong) : CborBoxedValue<ULong> {
...
@ExperimentalUnsignedTypes
@JvmInline
value class CborNegativeInteger(override val value: ULong) : CborBoxedValue<ULong> {
    override fun writeAsCbor(out: ByteArrayOutputStream) {
...
@ExperimentalUnsignedTypes
@JvmInline
value class CborLong(override val value: Long) : CborBoxedValue<Long> {
    override fun writeAsCbor(out: ByteArrayOutputStream) {
        if (value >= 0)
...
@ExperimentalUnsignedTypes
@JvmInline
value class CborTextString(override val value: String) : CborBoxedValue<String> {
    override fun writeAsCbor(out: ByteArrayOutputStream) {
...
@ExperimentalUnsignedTypes
data class CborByteString(override val value: ByteArray) : CborBoxedValue<ByteArray> {
    override fun writeAsCbor(out: ByteArrayOutputStream) {
...
@ExperimentalUnsignedTypes
data class CborArray(override val value: Array<CborValue>) : CborBoxedValue<Array<CborValue>> {
    override fun writeAsCbor(out: ByteArrayOutputStream) {
...
@ExperimentalUnsignedTypes
@JvmInline
value class CborTextStringMap(override val value: Map<String, CborValue>) :
    CborBoxedValue<Map<String, CborValue>> {
    override fun writeAsCbor(out: ByteArrayOutputStream) {
        CborMap(value.mapKeys { CborTextString(it.key) }).writeAsCbor(out)
    }
}

@ExperimentalUnsignedTypes
@JvmInline
value class CborLongMap(override val value: Map<Long, CborValue>) :
    CborBoxedValue<Map<Long, CborValue>> {
    override fun writeAsCbor(out: ByteArrayOutputStream) {
        CborMap(value.mapKeys { CborLong(it.key) }).writeAsCbor(out)
    }
}

@ExperimentalUnsignedTypes
@JvmInline
value class CborMap(override val value: Map<CborValue, CborValue>) :
...
@ExperimentalUnsignedTypes
@JvmInline
value class CborSimpleValue(override val value: UByte) : CborBoxedValue<UByte> {
    override fun writeAsCbor(out: ByteArrayOutputStream) {
        if (value >= 32U)
...
@ExperimentalUnsignedTypes
@JvmInline
value class CborFloatingPointNumber(override val value: Double) : CborBoxedValue<Double> {
    override fun writeAsCbor(out: ByteArrayOutputStream) {
...
@ExperimentalUnsignedTypes
@JvmInline
value class CborBoolean(override val value: Boolean) : CborBoxedValue<Boolean> {
    override fun writeAsCbor(out: ByteArrayOutputStream) {
        if (value) out.write(SIMPLE_VALUE_TRUE.toInt())
        else out.write(SIMPLE_VALUE_FALSE.toInt())
    }
}
...
object CborUndefined : CborBoxedValue<Nothing> {
    override val value get() = throw UninitializedPropertyAccessException()
    override fun writeAsCbor(out: ByteArrayOutputStream) {
...
private fun UShort.toBytes(): ByteArray =
    ByteBuffer.allocate(Short.SIZE_BYTES).putShort(this.toShort()).array()

@ExperimentalUnsignedTypes
private fun UInt.toBytes(): ByteArray =
    ByteBuffer.allocate(Int.SIZE_BYTES).putInt(this.toInt()).array()

@ExperimentalUnsignedTypes
private fun ULong.toBytes(): ByteArray =
    ByteBuffer.allocate(Long.SIZE_BYTES).putLong(this.toLong()).array()
